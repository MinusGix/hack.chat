var frontpage = [
	...
	"",
	"Original Source GitHub: https://github.com/AndrewBelt/hack.chat",
	"",
...
function pushMessage(args) {
	...
	if (args.admin) {
		...
		} else if (args.cmd === 'alert') {
			messageEl.classList.add('alert');
		}
	} else if (args.cmd === 'notify') {
		messageEl.classList.add('notify');
	} else if (args.chanowner) {
		messageEl.classList.add('chanowner');
	} else if (args.chanadmin) {
		messageEl.classList.add('chanadmin');
	} else if (args.chanmod) {
		messageEl.classList.add('chanmod');
	}
	...
	if (typeof (args.color) === 'string') {
		messageEl.style.color = args.color; //lets me specify the color
	}
	if (args.nick) {
		...
		if (args.cmd === 'notify') {
			nickLinkEl.onclick = function () {
				insertAtCursor('/whisper ' + args.nick + ' ');
				$('#chatinput').focus();
			};
		} else {
			nickLinkEl.onclick = function () {
...
$('#chatinput').onkeydown = function (e) {
	if (e.keyCode == 13 /* ENTER */ && !e.shiftKey) {
		...
		var index = text2.lastIndexOf('@', pos);
		if (index >= 0) {
			var stub = text2.substring(index + 1, pos).toLowerCase();
